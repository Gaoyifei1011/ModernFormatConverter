...
namespace ModernFormatConverter.Views.Pages
{
    ...
    {
        private readonly string TitleString = ResourceService.PhotoConversionResource.GetString("Title");
        private readonly SynchronizationContext synchronizationContext = SynchronizationContext.Current;
        private readonly OverlappedPresenter overlappedPresenter;
        private readonly SUBCLASSPROC photoConversionWindowSubClassProc;
        ...
        public PhotoConversionPage(AppWindow appWindow)
        {
            InitializeComponent();
            ...
            // 为应用主窗口添加窗口过程
            photoConversionWindowSubClassProc = new SUBCLASSPROC(PhotoConversionWindowSubClassProc);
            Comctl32Library.SetWindowSubclass((nint)AppWindow.Id.Value, photoConversionWindowSubClassProc, 0, 0);
        ...
        /// 应用窗口消息处理
        /// </summary>
        private nint PhotoConversionWindowSubClassProc(nint hWnd, WindowMessage Msg, nuint wParam, nint lParam, uint uIdSubclass, nint dwRefData)
        {
            switch (Msg)
            {
                // 窗口位置发生变化时触发的消息
                ...
                case WindowMessage.WM_ACTIVATE:
                    {
                        synchronizationContext.Post((_) =>
                        {
                            ...
                            {
                                LogService.WriteLog(TraceEventType.Error, nameof(ModernFormatConverter), nameof(PhotoConversionPage), nameof(PhotoConversionWindowSubClassProc), 1, e);
                            }
                        }, null);
                        break;
                    }
                ...
                case WindowMessage.WM_CLOSE:
                    {
                        synchronizationContext.Post(async (_) =>
                        {
                            ...
                            inputKeyboardSource.SystemKeyDown -= OnSystemKeyDown;
                            inputPointerSource.PointerReleased -= OnPointerReleased;
                            Comctl32Library.RemoveWindowSubclass((nint)AppWindow.Id.Value, photoConversionWindowSubClassProc, 0);
