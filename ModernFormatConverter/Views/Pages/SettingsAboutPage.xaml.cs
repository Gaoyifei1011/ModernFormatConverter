    ...
    public sealed partial class SettingsAboutPage : Page, INotifyPropertyChanged
    {
        ...
        private void OnShowReleaseNotesClicked(object sender, RoutedEventArgs args)
        {
            Task.Run(() =>
            {
                ...
                {
                    LogService.WriteLog(TraceEventType.Error, nameof(ModernFormatConverter), nameof(SettingsAboutPage), nameof(OnShowReleaseNotesClicked), 1, e);
                }
            });
        }
        ...
        private void OnSystemInformationClicked(object sender, RoutedEventArgs args)
        {
            Task.Run(() =>
            {
                ...
                {
                    LogService.WriteLog(TraceEventType.Error, nameof(ModernFormatConverter), nameof(SettingsAboutPage), nameof(OnSystemInformationClicked), 1, e);
                }
            });
        }
        ...
        private void OnHelpTranslateClicked(object sender, RoutedEventArgs args)
        {
            Task.Run(() =>
            {
                try
                {
                    Process.Start("https://github.com/Gaoyifei1011/ModernFormatConverter/issues");
                }
                catch (Exception e)
                {
                    LogService.WriteLog(TraceEventType.Error, nameof(ModernFormatConverter), nameof(SettingsAboutPage), nameof(OnHelpTranslateClicked), 1, e);
                }
            });
        }
        ...
        private void OnProjectDescriptionClicked(object sender, RoutedEventArgs args)
        {
            Task.Run(() =>
            {
                try
                {
                    Process.Start("https://github.com/Gaoyifei1011/ModernFormatConverter");
                }
                catch (Exception e)
                {
                    LogService.WriteLog(TraceEventType.Error, nameof(ModernFormatConverter), nameof(SettingsAboutPage), nameof(OnProjectDescriptionClicked), 1, e);
                }
            });
        }
        ...
        private void OnSendFeedbackClicked(object sender, RoutedEventArgs args)
        {
            Task.Run(() =>
            {
                ...
                {
                    LogService.WriteLog(TraceEventType.Error, nameof(ModernFormatConverter), nameof(SettingsAboutPage), nameof(OnSendFeedbackClicked), 1, e);
                }
            });
        }
        ...
        private static bool IsNetworkConnected()
        {
            ...
            {
                LogService.WriteLog(TraceEventType.Error, nameof(ModernFormatConverter), nameof(SettingsAboutPage), nameof(IsNetworkConnected), 1, e);
                return false;
            }
        }
    }
