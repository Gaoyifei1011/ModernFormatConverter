...
namespace ModernFormatConverter.Views.Pages
{
    ...
    {
        private readonly string TitleString = ResourceService.AudioConversionResource.GetString("Title");
        private readonly SynchronizationContext synchronizationContext = SynchronizationContext.Current;
        private readonly OverlappedPresenter overlappedPresenter;
        private readonly SUBCLASSPROC audioConversionWindowSubClassProc;
        ...
        public AudioConversionPage(AppWindow appWindow)
        {
            InitializeComponent();
            ...
            // 为应用主窗口添加窗口过程
            audioConversionWindowSubClassProc = new SUBCLASSPROC(AudioConversionWindowSubClassProc);
            Comctl32Library.SetWindowSubclass((nint)AppWindow.Id.Value, audioConversionWindowSubClassProc, 0, 0);
        ...
        /// 应用窗口消息处理
        /// </summary>
        private nint AudioConversionWindowSubClassProc(nint hWnd, WindowMessage Msg, nuint wParam, nint lParam, uint uIdSubclass, nint dwRefData)
        {
            switch (Msg)
            {
                // 窗口位置发生变化时触发的消息
                ...
                case WindowMessage.WM_ACTIVATE:
                    {
                        synchronizationContext.Post((_) =>
                        {
                            ...
                            {
                                LogService.WriteLog(TraceEventType.Error, nameof(ModernFormatConverter), nameof(AudioConversionPage), nameof(AudioConversionWindowSubClassProc), 1, e);
                            }
                        }, null);
                        break;
                    }
                ...
                case WindowMessage.WM_CLOSE:
                    {
                        synchronizationContext.Post(async (_) =>
                        {
                            ...
                            inputKeyboardSource.SystemKeyDown -= OnSystemKeyDown;
                            inputPointerSource.PointerReleased -= OnPointerReleased;
                            Comctl32Library.RemoveWindowSubclass((nint)AppWindow.Id.Value, audioConversionWindowSubClassProc, 0);
