...
namespace ModernFormatConverter.WindowsAPI.ComTypes
{
    ...
    public interface IDXGIAdapter
    {
        ...
        [PreserveSig]
        int SetPrivateDataInterface(in Guid Name, nint pUnknown);
        ...
        /// <returns>返回以下 DXGI_ERROR之一。</returns>
        [PreserveSig]
        int GetPrivateData(in Guid Name, ref uint pDataSize, out nint pData);
        ...
        /// <returns>返回 DXGI_ERROR 值之一。</returns>
        [PreserveSig]
        int GetParent(in Guid riid, out nint ppParent);
        ...
        [PreserveSig]
        int CheckInterfaceSupport(in Guid InterfaceName, out nint pUMDVersion);
    }
}
